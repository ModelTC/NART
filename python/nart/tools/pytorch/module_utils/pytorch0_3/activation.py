...
class PReLU(Function):
    ...
    def forward(ctx, input, weight):
        ...
        ctx._backend.PReLU_updateOutput(
            ctx._backend.library_state, input, output, weight
        )
...
class PReLUBackward(Function):
    ...
    def forward(ctx, input, weight, grad_output, backend):
        ...
        backend.PReLU_updateGradInput(
            backend.library_state, input, grad_output, grad_input, weight
        )
...
class RReLU(InplaceFunction):
    ...
    def backward(ctx, grad_output):
        ...
        return (
            RReLUBackward.apply(
                input, grad_output, ctx.noise, ctx.lower, ctx.upper, ctx.train
            ),
            None,
            None,
            None,
    ...
    def backward(ctx, grad_output):
        ...
        if grad_output.volatile:
            ...
            grad_input = (
                grad_output
                * SELU.scale
                * (positive_mask + negative_mask * (output / SELU.scale + SELU.alpha))
            )
