...
def true_divide(g, self, other):
    ...
    if sym_help._is_fp(other):
        self = g.op(
            "Cast", self, to_i=sym_help.cast_pytorch_to_onnx[other.type().scalarType()]
        )
        return g.op("Div", self, other)
...
def _max_pool(name, tuple_fn, ndims, return_indices):
    ...
    def symbolic_fn(g, input, kernel_size, stride, padding, dilation, ceil_mode):
        ...
        if ceil_mode:
            ...
            padding = padding * 2
            warnings.warn(
                "[MaxPool] with ceil_mode=True is not supported by ONNX.OPSET-9, we nart extend its implements by default, "
                "which may cause the exported onnx model not standard."
            )
        else:
            padding = padding * 2
        kwargs = {
...
def _avg_pool(name, tuple_fn):
    ...
    def symbolic_fn(
        ...
        if count_include_pad:
            warnings.warn(
                "Pooling with count_include_pad=True is not supported in caffe."
            )
        ...
        if ceil_mode:
            ...
            padding = padding * 2
            warnings.warn(
                "[AvgPool] with ceil_mode=True is not supported by ONNX.OPSET-9, we nart extend its implements by default, "
                "which may cause the exported onnx model not standard."
            )
        else:
            padding = padding * 2
    ...
    def symbolic_fn(g, input, output_size):
        ...
        if mod != [0] * len(mod):
            ...
            if (
                ...
                return sym_help._onnx_unsupported(
                    name + ", since output size is not factor of input size"
                )
...
def _interpolate(name, dim, interpolate_mode):
    def symbolic_fn(g, input, output_size, *args):
        ...
        if interpolate_mode == "linear" and not align_corners:
            return _unimplemented(name, "align_corners == False")
        if scales is None:
...
def type_as(g, self, other):
    ...
    if other_dtype is not None:
        ...
        if (
            ...
            raise RuntimeError(
                "Unsupported: ONNX export of type_as for tensor " "of unknown dtype."
            )
...
def cosine_similarity(g, x1, x2, dim, eps):
    if (
        ...
        return g.op(
            "ATen", x1, x2, dim_i=dim, eps_f=eps, operator_s="cosine_similarity"
        )
    else:
...
def hardswish(g, self):
    ...
    hardtanh_ = sym_help._hardtanh_helper(
        g,
        input,
    ...
    hardtanh_ = g.op(
        "Div", hardtanh_, g.op("Constant", value_t=torch.tensor(6, dtype=torch.float))
    )
...
def pixel_shuffle(g, self, upscale_factor):
    ...
    if any([i is None for i in dims[1:]]):
        return _unimplemented(
            "pixel_shuffle", "only support static input shape, except for batch size"
        )
...
def arange(g, *args):
    ...
    if len(args) == 2:
        ...
        arange_tensor = sym_help._squeeze_helper(
            g, nonzero(g, ones(g, end, dtype, None, None)), [1]
        )
        ...
        arange_tensor = sym_help._squeeze_helper(
            g, nonzero(g, ones(g, end, dtype, *(args[2:]))), [1]
        )
        return g.op("Cast", arange_tensor, to_i=sym_help.scalar_type_to_onnx[dtype])
    elif len(args) == 6:
        ...
        range_tensor = g.op("Div", g.op("Sub", end, start), step)
        arange_tensor = sym_help._squeeze_helper(
            g, nonzero(g, ones(g, range_tensor, dtype, *(args[4:]))), [1]
        )
        arange_tensor = g.op("Add", g.op("Mul", arange_tensor, step), start)
        return g.op("Cast", arange_tensor, to_i=sym_help.scalar_type_to_onnx[dtype])
    else:
...
def meshgrid(g, tensor_list):
    tensors = [
        view(g, t, g.op("Constant", value_t=torch.LongTensor([-1])))
        for t in sym_help._unpack_list(tensor_list)
    ]
