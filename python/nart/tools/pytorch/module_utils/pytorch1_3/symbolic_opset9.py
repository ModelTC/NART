...
def _max_pool(name, tuple_fn, ndims, return_indices):
    ...
    def symbolic_fn(g, input, kernel_size, stride, padding, dilation, ceil_mode):
        ...
        # use ceil_mode attr to indicate ceil_mode, not padding
        padding = padding * 2
        kwargs = {
...
def index_fill(g, self, dim, index, value):
    ...
    unsqueezed_index = g.op(
        "Unsqueeze", index, axes_i=[i for i in range(self_dim) if i != dim_value]
    )
    expanded_index_shape = g.op(
        "Scatter",
        g.op("Shape", self),
...
def index(g, self, index):
    ...
    if len(indices) == 1:
        ...
        adv_idx_indices = [
            i for i, idx in enumerate(indices) if not idx.node().mustBeNone()
        ]
