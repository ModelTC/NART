...
def zeros(g, sizes, dtype, layout, device):
    ...
    return g.op(
        "ConstantFill",
        sizes,
        dtype_i=scalar_type_to_onnx[dtype],
        input_as_shape_i=1,
        value_f=0,
    )
...
def full_like(g, input, fill_value, dtype, layout, device):
    return g.op(
        "ConstantLike", input, dtype_i=scalar_type_to_onnx[dtype], value_f=fill_value
    )
...
def pixel_shuffle(g, self, upscale_factor):
    ...
    after_view = view(
        g, self, [-1, upscale_factor, upscale_factor, output_channel, dims[2], dims[3]]
    )
