...
def _get_const(value, desc, arg_name):
    if _is_value(value) and value.node().kind() != "onnx::Constant":
        raise RuntimeError(
            "ONNX symbolic expected a constant value of the {} argument".format(
                arg_name
            )
        )
    return _parse_arg(value, desc)
...
def _if_scalar_type_as(g, self, tensor):
    ...
    if isinstance(self, torch._C.Value):
        ...
        tensor.type().kind() == "DimensionedTensorType"
        or tensor.type().kind() == "CompleteTensorType"
    ):
...
def _reduce_op_symbolic(onnx_op_name):
    def symbolic(g, self, dim=None, keepdim=None):
        if dim is None:
            ...
            # dim-reduce path
            dim, keepdim = _get_const(dim, "i", "dim"), _get_const(
                keepdim, "i", "keepdim"
            )
...
def select(g, self, dim, index):
    if dim > 1:
        ...
        slice_node = _slice_op(
            g, self, axes=[dim], starts=[index_val], ends=[index_val + 1]
        )
...
def squeeze(g, self, dim=None):
    if dim is None:
        ...
        for i, dim in enumerate(dims):
            if dim < 0:
                if (
                    self.type().kind() == "CompleteTensorType"
                    or self.type().kind() == "DimensionedTensorType"
                ):
                    warnings.warn(
...
def softmax(g, input, dim, dtype=None):
    ...
    if (
        input.type().kind() == "CompleteTensorType"
        or input.type().kind() == "DimensionedTensorType"
    ):
        if dim < 0:
            dim = input.type().dim() + dim
        if input.type().dim() != dim + 1:
            warnings.warn(
                "[Softmax] PyTorch and ONNX use different strategies to split input tensor into vectors."
                + "Thus dim and axis have different meaning, which may cause precision loss."
            )
        ...
        return softmax
    warnings.warn(
        "[Softmax] Wrong input type. Expected `CompleteTensorType` or `DimensionedTensorType`"
        + ", bug got "
        + input.type().kind()
        + ". PyTorch will export Exp ReduceSum Div instead of Softmax."
    )
    exp = g.op("Exp", input)
    ...
    def symbolic_fn(g, input, kernel_size, stride, padding, dilation, ceil_mode):
        ...
        if return_indices:
            r, indices = g.op(
                ...
                ceil_mode_i=1 if ceil_mode else 0,
            )
...
def _avg_pool(name, tuple_fn):
    ...
    def symbolic_fn(
        ...
        if ceil_mode:
            padding_ceil = get_pool_ceil_padding(input, kernel_size, stride, padding)
        # dont generate Pad if all pads are zero
        ...
        output = g.op(
            ...
            strides_i=tuple_fn(stride),
            pads_i=padding,
            count_include_pad_i=1 if count_include_pad else 0,
            ceil_mode_i=1 if ceil_mode else 0,
        )
        return output
...
def _adaptive_pool(name, type, tuple_fn, fn=None):
    ...
    def symbolic_fn(g, input, output_size):
        ...
        if type == "MaxPool":
            return fn(g, input, k, s, (0,) * len(dim), (1,) * len(dim), False)
        output = g.op(
            type,
            input,
            kernel_shape_i=tuple_fn(k),
            strides_i=tuple_fn(s),
            pads_i=tuple_fn((0,) * len(dim)),
            ceil_mode_i=False,
        )
        return output
...
# overwritten by neighbor interpolate.py
@parse_args("v", "is")
def upsample_nearest2d(g, input, output_size):
    ...
    return g.op(
        "Upsample",
        input,
        width_scale_f=width_scale,
        height_scale_f=height_scale,
        mode_s="nearest",
    )
...
def upsample_bilinear2d(g, input, output_size, align_corners):
    if not align_corners:
        return _unimplemented("upsample_bilinear2d", "align_corners == False")
    height_scale = float(output_size[-2]) / input.type().sizes()[-2]
    width_scale = float(output_size[-1]) / input.type().sizes()[-1]
    return g.op(
        "Upsample",
...
def log_softmax(g, input, dim=None, dtype=None):
    ...
    if input.type().dim() != dim + 1:
        return _unimplemented(
            "dim", "ONNX and PyTorch use different strategies to split the input."
        )
...
def min(g, self, dim_or_y=None, keepdim=None):
    ...
    if keepdim is None:
        ...
        # TODO: export it as ReduceMax
        return g.op(
            "ATen", self, operator_s="min", dim_i=dim, keepdim_i=keepdim, outputs=2
        )
    ...
    if _is_value(const_value):
        ...
        return g.op(
            ...
            value_t=torch.tensor(
                [const_value], dtype=scalar_type_to_pytorch_type[dtype]
            ),
        )
...
def _generic_rnn(
    ...
    def retrieve_state(x, start, end):
        return (
            x
            if num_layers == 1
            else g.op("Slice", x, axes_i=[0], starts_i=[start], ends_i=[end])
        )
