...
def quant_weights(args, model):
    ...
    for i in model.layer:
        if (
            ...
            wmax = max(np.array(i.blobs[0].data).max(), 0)
            wmin = min(np.array(i.blobs[0].data).min(), 0)
...
def quant_activation(args, model, net):
    ...
    for data in input_data_generator(args.input_dir, inputs, data_num=args.data_num):
        ...
        for k in net.blobs:
            act_dict[k]["max"].append(float(net.blobs[k].data.max()))
            act_dict[k]["min"].append(float(net.blobs[k].data.min()))
    ...
    if args.symmetric:
        ...
        for k in act_dict:
            ...
            # calc final max/min
            # act_dict[k]['max'] = np.average(act_dict[k]['max'])
            # act_dict[k]['min'] = np.average(act_dict[k]['min'])
