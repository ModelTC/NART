...
class Shape(ONNXNodeParser, OpDesc, layertype="Shape", parser=real_parser):
    @classmethod
    def _parse_(cls, node, network, itensor_by_name):
        ipt = itensor_by_name[node.input[0]]
        shape_layer = network.add_shape(ipt)
        shape_layer.name = node.name

        outputs = {node.output[0]: shape_layer.get_output(0)}
        return shape_layer, outputs
