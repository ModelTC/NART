...
#ifndef TURING_INDIRECT_CONV_WMMA_INT4_LDG16_K64_CUH
#define TURING_INDIRECT_CONV_WMMA_INT4_LDG16_K64_CUH
...
__global__ void lb_turing_indirect_conv_wmma_int4_ldg16_k64_relu(
...

{
    ...
    const int MATRIX_A_SHMEM_STRIDE = 64 / 2; // WMMA_16x16_M * WMMA_16x16_K;//WMMA_K_STEP_CHANGE *
                                              // WMMA_16x16_K + SHMEM_CONFILCT_AVOID;
    const int MATRIX_B_SHMEM_STRIDE = 64 / 2; // WMMA_16x16_N * WMMA_16x16_K;
    ...
    uint32_t laneId_0_1 = threadIdx.x & 1;
...

    {
        ...
        int matrx_B_warp_col = matrix_B_each_warp_actual_col_base + laneId_even; //+ laneId_0_15;
        int matrx_A_warp_row = matrix_A_each_warp_actual_row_base + laneId_even; //+ laneId_0_15;

        preComputeMatixA
            MatrixAInfo[WMMA_ROW_TOTAL_SZ * WMMA_16x16_M / (WARP_PER_BLOCK_CHANGE * 16)]; // 16
#pragma unroll
        for (int jj = 0; jj < WMMA_ROW_TOTAL_SZ * WMMA_16x16_M / (16 * WARP_PER_BLOCK_CHANGE);
             jj++) { // 16
                ...
                for (int channel_cnt = 0; channel_cnt < i_c; channel_cnt += K_step) {
                    int4 A_row_frag
                        [WMMA_ROW_TOTAL_SZ * WMMA_16x16_M
                         / (16 * WARP_PER_BLOCK_CHANGE)]; //[WMMA_ROW_TOTAL_SZ * WMMA_16x16_M /
                                                          //WARP_PER_BLOCK_CHANGE];
                    int4 B_row_frag
                        [WMMA_COL_TOTAL_SZ * WMMA_16x16_N
                         / (16 * WARP_PER_BLOCK_CHANGE)]; //[WMMA_COL_TOTAL_SZ * WMMA_16x16_N /
                                                          //WARP_PER_BLOCK_CHANGE];

                    // drag matrix weight, 1 warp 1 line(8x16), each thread drag int
#pragma unroll
                    for (int jj = 0;
                         jj < WMMA_COL_TOTAL_SZ * WMMA_16x16_N / (16 * WARP_PER_BLOCK_CHANGE);
                         jj++) {
                        // jj need be filled
                        int4 *src = ((
                            int4
                                *)(weight_tensor + matrx_B_warp_col * GEMM_K_PAD / 2 + laneId_0_1 * 16 + jj * 16 * GEMM_K_PAD / 2 + (k_h_index * k_w + k_w_index) * i_c / 2 + channel_cnt / 2)); // laneId_half * 16
                        bool out_filled_zero = (matrx_B_warp_col + jj * 16) >= out_channel;
                        int4 *src_t = out_filled_zero ? (int4 *)zero_arr : src;
                        B_row_frag[jj] = __ldg(src_t);
                    }

#pragma unroll
                    for (int jj = 0;
                         jj < WMMA_ROW_TOTAL_SZ * WMMA_16x16_M / (16 * WARP_PER_BLOCK_CHANGE);
                         jj++) {
                        // jj need be filled
                        ...
                        int4 *src = ((
                            int4
                                *)(input_tensor + image_offset / 2 + k_h_index * (i_w * i_c / 2) + k_w_index * i_c / 2 + channel_cnt / 2 + laneId_0_1 * 16)); // laneId_half * 16
...
#pragma unroll
                        for (int jj = 0;
                            ...
                            int warp_sharedmm_offset = warpId
                                ...
                                + jj * 16 * MATRIX_B_SHMEM_STRIDE;
                            int lane_sharedmm_offset = laneId_0_1 * 16
                                + laneId_even * MATRIX_B_SHMEM_STRIDE; // laneId_even * 16 +
...
#pragma unroll
                        for (int jj = 0;
                            ...
                            int warp_sharedmm_offset = warpId
                                ...
                                + jj * 16 * MATRIX_A_SHMEM_STRIDE;
                            int lane_sharedmm_offset = laneId_0_1 * 16
                                + laneId_even * MATRIX_A_SHMEM_STRIDE; // laneId_even * 16 +
                                                                       // laneId_ldg_cacheline *
                                                                       // MATRIX_B_SHMEM_STRIDE;//
                    ...
                    if (i > cnt) {
                        ...
                        int2 my_A_frag0[WARP_ROW_WMMA_SZ_CHANGE];
                        int2 my_A_frag1[WARP_ROW_WMMA_SZ_CHANGE];
                        int2 my_B_frag0[WARP_COL_WMMA_SZ_CHANGE];
                        int2 my_B_frag1[WARP_COL_WMMA_SZ_CHANGE];
#pragma unroll
...
#pragma unroll
                            for (int row = 0; row < WARP_ROW_WMMA_SZ_CHANGE; row++) {
                                int warp_offset = (warpIdy * WARP_ROW_WMMA_SZ_CHANGE + row)
                                    ...
                                    + k;
                                int lane_offset
                                    = laneId_0_4 * 8 + laneId_eighth * MATRIX_A_SHMEM_STRIDE;
                                int8_t *a_frag0_ptr = (int8_t *)shmem_ptr;
                                int8_t *a_frag1_ptr = a_frag0_ptr + 8 * MATRIX_A_SHMEM_STRIDE;
                                my_A_frag0[row]
                                    = *((int2 *)(&a_frag0_ptr[warp_offset + lane_offset]));
                                my_A_frag1[row]
                                    = *((int2 *)(&a_frag1_ptr[warp_offset + lane_offset]));
                            }
...
#pragma unroll
                            for (int col = 0; col < WARP_COL_WMMA_SZ_CHANGE; col++) {
                                int warp_offset = (warpIdx * WARP_COL_WMMA_SZ_CHANGE + col)
                                    ...
                                    + k;
                                int lane_offset
                                    = laneId_0_4 * 8 + laneId_eighth * MATRIX_B_SHMEM_STRIDE;
                                int8_t *b_frag0_ptr = (int8_t *)shmem_ptr + Matrix_B_shmem_offset;
                                int8_t *b_frag1_ptr = b_frag0_ptr + 8 * MATRIX_B_SHMEM_STRIDE;
                                my_B_frag0[col]
                                    = *((int2 *)(&b_frag0_ptr[warp_offset + lane_offset]));
                                my_B_frag1[col]
                                    = *((int2 *)(&b_frag1_ptr[warp_offset + lane_offset]));
                            }
...
#pragma unroll
                for (int row = 0; row < WARP_ROW_WMMA_SZ_CHANGE; row++) {
                    int warp_offset = (warpIdy * WARP_ROW_WMMA_SZ_CHANGE + row) * WMMA_16x16_M
                            * MATRIX_A_SHMEM_STRIDE
                        + k;
                    int lane_offset = laneId_0_4 * 8 + laneId_eighth * MATRIX_A_SHMEM_STRIDE;
                    int8_t *a_frag0_ptr = (int8_t *)shmem_ptr;
                    int8_t *a_frag1_ptr = a_frag0_ptr + 8 * MATRIX_A_SHMEM_STRIDE;
                    my_A_frag0[row] = *((int2 *)(&a_frag0_ptr[warp_offset + lane_offset]));
                    my_A_frag1[row] = *((int2 *)(&a_frag1_ptr[warp_offset + lane_offset]));
                }
...
#pragma unroll
                for (int col = 0; col < WARP_COL_WMMA_SZ_CHANGE; col++) {
                    int warp_offset = (warpIdx * WARP_COL_WMMA_SZ_CHANGE + col) * WMMA_16x16_N
                            * MATRIX_B_SHMEM_STRIDE
                        + k;
                    int lane_offset = laneId_0_4 * 8 + laneId_eighth * MATRIX_B_SHMEM_STRIDE;
                    int8_t *b_frag0_ptr = (int8_t *)shmem_ptr + Matrix_B_shmem_offset;
                    int8_t *b_frag1_ptr = b_frag0_ptr + 8 * MATRIX_B_SHMEM_STRIDE;
                    my_B_frag0[col] = *((int2 *)(&b_frag0_ptr[warp_offset + lane_offset]));
                    my_B_frag1[col] = *((int2 *)(&b_frag1_ptr[warp_offset + lane_offset]));
                }
...
#pragma unroll
                    for (int col = 0; col < WARP_COL_WMMA_SZ_CHANGE; col++) {
                        my_c_frag[row][col][0]
                            = mma8832(my_c_frag[row][col][0], my_A_frag0[row].x, my_B_frag0[col].x);
                        my_c_frag[row][col][0]
                            = mma8832(my_c_frag[row][col][0], my_A_frag0[row].y, my_B_frag0[col].y);

                        my_c_frag[row][col][1]
                            = mma8832(my_c_frag[row][col][1], my_A_frag0[row].x, my_B_frag1[col].x);
                        my_c_frag[row][col][1]
                            = mma8832(my_c_frag[row][col][1], my_A_frag0[row].y, my_B_frag1[col].y);

                        my_c_frag[row][col][2]
                            = mma8832(my_c_frag[row][col][2], my_A_frag1[row].x, my_B_frag0[col].x);
                        my_c_frag[row][col][2]
                            = mma8832(my_c_frag[row][col][2], my_A_frag1[row].y, my_B_frag0[col].y);

                        my_c_frag[row][col][3]
                            = mma8832(my_c_frag[row][col][3], my_A_frag1[row].x, my_B_frag1[col].x);
                        my_c_frag[row][col][3]
                            = mma8832(my_c_frag[row][col][3], my_A_frag1[row].y, my_B_frag1[col].y);
                    }
                }
            }
        }
...
#pragma unroll
        for (int j = 0; j < WARP_COL_WMMA_SZ_CHANGE; j++) {
            ...
            float2 *bias_channel_reg0_ptr
                = bias_flag ? (float2 *)(bias + channel_id_frag_reg0) : (float2 *)zero_arr;
            float2 *bias_channel_reg4_ptr
                = bias_flag ? (float2 *)(bias + channel_id_frag_reg4) : (float2 *)zero_arr;

            bias_channel_reg0[j] = __ldg(bias_channel_reg0_ptr);
            bias_channel_reg4[j] = __ldg(bias_channel_reg4_ptr);
        }
...
void lb_turing_indirect_conv_wmma_int4_ldg16_k64_singlebuf_relu(
    ...
    workspace_t *ws)
{
    ...
    cudaFuncSetAttribute(
        lb_turing_indirect_conv_wmma_int4_ldg16_k64_relu<
...
template <
    ...
    int WMMA_16x16_M, int WMMA_16x16_N, int WMMA_16x16_K, int BLOCK_ROW_WARP_NUM_CHANGE,
    int BLOCK_COL_WARP_NUM_CHANGE, bool overlap>
__global__ void lb_turing_indirect_conv_wmma_int4_ldg16_k64_split3x3_relu(
...
template <
    int M_TILED, int N_TILED, int K_TILED, int WMMA_16x16_M, int WMMA_16x16_N, int WMMA_16x16_K,
    int BLOCK_ROW_WARP_NUM_CHANGE, int BLOCK_COL_WARP_NUM_CHANGE, bool OVERLAP>
void lb_turing_indirect_conv_wmma_int4_ldg16_k64_split3x3_singlebuf_relu(
    ...
    void *preComputeA, workspace_t *ws)
{
    ...
    dim3 blockDim(WARP_SIZE, BLOCK_COL_WARP_NUM_CHANGE, BLOCK_ROW_WARP_NUM_CHANGE);
    cudaFuncSetAttribute(
        lb_turing_indirect_conv_wmma_int4_ldg16_k64_split3x3_relu<
            WARP_ROW_WMMA_SZ_CHANGE, WARP_COL_WMMA_SZ_CHANGE, WMMA_K_STEP_CHANGE, WMMA_16x16_M,
            WMMA_16x16_N, WMMA_16x16_K, BLOCK_ROW_WARP_NUM_CHANGE, BLOCK_COL_WARP_NUM_CHANGE,
            OVERLAP>,
        cudaFuncAttributeMaxDynamicSharedMemorySize, 65536);
    ...
    lb_turing_indirect_conv_wmma_int4_ldg16_k64_split3x3_relu<
        ...
        <<<gridDim, blockDim, sharedMM_AB_sz, CUDA_WORKSPACE_STREAM(ws)>>>(
            ...
            preComputeA_ptr);
    quantize<<<GEMM_M, out_channel / 2, 0, CUDA_WORKSPACE_STREAM(ws)>>>(
