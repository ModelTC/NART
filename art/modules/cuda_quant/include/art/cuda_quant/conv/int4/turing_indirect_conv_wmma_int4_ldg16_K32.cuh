...
#ifndef TURING_INDIRECT_CONV_WMMA_INT4_LDG16_K32_CUH
#define TURING_INDIRECT_CONV_WMMA_INT4_LDG16_K32_CUH

// warning: this function should SET M_TILE >= 128!!
...
__global__ void lb_turing_indirect_conv_wmma_int4_ldg16_k32_relu(
...

{
    ...
    const int MATRIX_A_SHMEM_STRIDE = 32 / 2; // WMMA_16x16_M * WMMA_16x16_K;//WMMA_K_STEP_CHANGE *
                                              // WMMA_16x16_K + SHMEM_CONFILCT_AVOID;
    const int MATRIX_B_SHMEM_STRIDE = 32 / 2; // WMMA_16x16_N * WMMA_16x16_K;
...

    {
        ...
        int matrx_B_warp_col = matrix_B_each_warp_actual_col_base + laneId; //+ laneId_0_15;
        int matrx_A_warp_row = matrix_A_each_warp_actual_row_base + laneId; //+ laneId_0_15;

        preComputeMatixA
            MatrixAInfo[WMMA_ROW_TOTAL_SZ * WMMA_16x16_M / (WARP_PER_BLOCK_CHANGE * 32)]; // 16
#pragma unroll
        for (int jj = 0; jj < WMMA_ROW_TOTAL_SZ * WMMA_16x16_M / (32 * WARP_PER_BLOCK_CHANGE);
             jj++) { // 16
            preComputeMatixA *MatrixA_info = preComputeA + matrx_A_warp_row + jj * 32;
                ...
                for (int channel_cnt = 0; channel_cnt < i_c; channel_cnt += K_step) {
                    int4 A_row_frag
                        [WMMA_ROW_TOTAL_SZ * WMMA_16x16_M
                         / (32 * WARP_PER_BLOCK_CHANGE)]; //[WMMA_ROW_TOTAL_SZ * WMMA_16x16_M /
                                                          //WARP_PER_BLOCK_CHANGE];
                    int4 B_row_frag
                        [WMMA_COL_TOTAL_SZ * WMMA_16x16_N
                         / (32 * WARP_PER_BLOCK_CHANGE)]; //[WMMA_COL_TOTAL_SZ * WMMA_16x16_N /
                                                          //WARP_PER_BLOCK_CHANGE];

                    // drag matrix weight, 1 warp 1 line(8x16), each thread drag int
#pragma unroll
                    for (int jj = 0;
                         jj < WMMA_COL_TOTAL_SZ * WMMA_16x16_N / (32 * WARP_PER_BLOCK_CHANGE);
                         jj++) {
                        // jj need be filled
                        int4 *src = ((
                            int4
                                *)(weight_tensor + matrx_B_warp_col * GEMM_K_PAD / 2 + laneId0 * 16 + jj * 32 * GEMM_K_PAD / 2 + (k_h_index * k_w + k_w_index) * i_c / 2 + channel_cnt / 2)); // laneId_half * 16
                        bool out_filled_zero = (matrx_B_warp_col + jj * 32) >= out_channel;
...
#pragma unroll
                    for (int jj = 0;
                         jj < WMMA_ROW_TOTAL_SZ * WMMA_16x16_M / (32 * WARP_PER_BLOCK_CHANGE);
                         jj++) {
                        // jj need be filled
                        ...
                        int4 *src = ((
                            int4
                                *)(input_tensor + image_offset / 2 + k_h_index * (i_w * i_c / 2) + k_w_index * i_c / 2 + channel_cnt / 2 + laneId0 * 16)); // laneId_half * 16
...
#pragma unroll
                        for (int jj = 0;
                            ...
                            int warp_sharedmm_offset = warpId
                                ...
                                + jj * 32 * MATRIX_B_SHMEM_STRIDE;
                            int lane_sharedmm_offset = laneId0 * 16
                                + laneId * MATRIX_B_SHMEM_STRIDE; // laneId_even * 16 +
...
#pragma unroll
                        for (int jj = 0;
                            ...
                            int warp_sharedmm_offset = warpId
                                ...
                                + jj * 32 * MATRIX_A_SHMEM_STRIDE;
                            int lane_sharedmm_offset = laneId0 * 16
                                + laneId * MATRIX_A_SHMEM_STRIDE; // laneId_even * 16 +
                                                                  // laneId_ldg_cacheline *
                                                                  // MATRIX_B_SHMEM_STRIDE;//
                    ...
                    if (i > cnt) {
                        ...
                        int my_A_frag0[WARP_ROW_WMMA_SZ_CHANGE];
                        int my_A_frag1[WARP_ROW_WMMA_SZ_CHANGE];
                        int my_B_frag0[WARP_COL_WMMA_SZ_CHANGE];
                        int my_B_frag1[WARP_COL_WMMA_SZ_CHANGE];
#pragma unroll
...
#pragma unroll
                            for (int row = 0; row < WARP_ROW_WMMA_SZ_CHANGE; row++) {
                                int warp_offset = (warpIdy * WARP_ROW_WMMA_SZ_CHANGE + row)
                                    ...
                                    + k;
                                int lane_offset
                                    = laneId_0_4 * 4 + laneId_eighth * MATRIX_A_SHMEM_STRIDE;
                                int8_t *a_frag0_ptr = (int8_t *)shmem_ptr;
                                int8_t *a_frag1_ptr = a_frag0_ptr + 8 * MATRIX_A_SHMEM_STRIDE;
                                my_A_frag0[row]
                                    = *((int *)(&a_frag0_ptr[warp_offset + lane_offset]));
                                my_A_frag1[row]
                                    = *((int *)(&a_frag1_ptr[warp_offset + lane_offset]));
                            }
...
#pragma unroll
                            for (int col = 0; col < WARP_COL_WMMA_SZ_CHANGE; col++) {
                                int warp_offset = (warpIdx * WARP_COL_WMMA_SZ_CHANGE + col)
                                    ...
                                    + k;
                                int lane_offset
                                    = laneId_0_4 * 4 + laneId_eighth * MATRIX_B_SHMEM_STRIDE;
                                int8_t *b_frag0_ptr = (int8_t *)shmem_ptr + Matrix_B_shmem_offset;
                                int8_t *b_frag1_ptr = b_frag0_ptr + 8 * MATRIX_B_SHMEM_STRIDE;
                                my_B_frag0[col]
                                    = *((int *)(&b_frag0_ptr[warp_offset + lane_offset]));
                                my_B_frag1[col]
                                    = *((int *)(&b_frag1_ptr[warp_offset + lane_offset]));
                            }
...
#pragma unroll
                                for (int col = 0; col < WARP_COL_WMMA_SZ_CHANGE; col++) {
                                    my_c_frag[row][col][0] = mma8832(
                                        my_c_frag[row][col][0], my_A_frag0[row], my_B_frag0[col]);

                                    my_c_frag[row][col][1] = mma8832(
                                        my_c_frag[row][col][1], my_A_frag0[row], my_B_frag1[col]);

                                    my_c_frag[row][col][2] = mma8832(
                                        my_c_frag[row][col][2], my_A_frag1[row], my_B_frag0[col]);

                                    my_c_frag[row][col][3] = mma8832(
                                        my_c_frag[row][col][3], my_A_frag1[row], my_B_frag1[col]);
                                }
                            }
                        }
                        __syncthreads();
...
#pragma unroll
                for (int row = 0; row < WARP_ROW_WMMA_SZ_CHANGE; row++) {
                    int warp_offset = (warpIdy * WARP_ROW_WMMA_SZ_CHANGE + row) * WMMA_16x16_M
                            * MATRIX_A_SHMEM_STRIDE
                        + k;
                    int lane_offset = laneId_0_4 * 4 + laneId_eighth * MATRIX_A_SHMEM_STRIDE;
                    int8_t *a_frag0_ptr = (int8_t *)shmem_ptr;
                    int8_t *a_frag1_ptr = a_frag0_ptr + 8 * MATRIX_A_SHMEM_STRIDE;
                    my_A_frag0[row] = *((int *)(&a_frag0_ptr[warp_offset + lane_offset]));
                    my_A_frag1[row] = *((int *)(&a_frag1_ptr[warp_offset + lane_offset]));
                }
...
#pragma unroll
                for (int col = 0; col < WARP_COL_WMMA_SZ_CHANGE; col++) {
                    int warp_offset = (warpIdx * WARP_COL_WMMA_SZ_CHANGE + col) * WMMA_16x16_N
                            * MATRIX_B_SHMEM_STRIDE
                        + k;
                    int lane_offset = laneId_0_4 * 4 + laneId_eighth * MATRIX_B_SHMEM_STRIDE;
                    int8_t *b_frag0_ptr = (int8_t *)shmem_ptr + Matrix_B_shmem_offset;
                    int8_t *b_frag1_ptr = b_frag0_ptr + 8 * MATRIX_B_SHMEM_STRIDE;
                    my_B_frag0[col] = *((int *)(&b_frag0_ptr[warp_offset + lane_offset]));
                    my_B_frag1[col] = *((int *)(&b_frag1_ptr[warp_offset + lane_offset]));
                }
...
#pragma unroll
                    for (int col = 0; col < WARP_COL_WMMA_SZ_CHANGE; col++) {
                        my_c_frag[row][col][0]
                            = mma8832(my_c_frag[row][col][0], my_A_frag0[row], my_B_frag0[col]);

                        my_c_frag[row][col][1]
                            = mma8832(my_c_frag[row][col][1], my_A_frag0[row], my_B_frag1[col]);

                        my_c_frag[row][col][2]
                            = mma8832(my_c_frag[row][col][2], my_A_frag1[row], my_B_frag0[col]);

                        my_c_frag[row][col][3]
                            = mma8832(my_c_frag[row][col][3], my_A_frag1[row], my_B_frag1[col]);
                    }
                }
            }
        }
...
void lb_turing_indirect_conv_wmma_int4_ldg16_k32_singlebuf_relu(
    ...
    workspace_t *ws)
{
    ...
    cudaFuncSetAttribute(
        lb_turing_indirect_conv_wmma_int4_ldg16_k32_relu<
