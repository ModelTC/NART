...
#ifndef TURING_INDIRECT_CONV_WMMA_INT4_LDG16_K128_CUH
#define TURING_INDIRECT_CONV_WMMA_INT4_LDG16_K128_CUH

// 尽管变量名依然沿用WMMA_16x16 实际计算矩阵大小为 16x32 16x32
template <
...
__global__ void lb_turing_indirect_conv_wmma_int4_ldg16_k128_matrixB_relu_ker(
    ...
    int out_width, uint8_t out_bits, float *alphas, preComputeMatixA *preComputeA)
{
    ...
    const int MATRIX_B_SHMEM_STRIDE = 128 / 2; // WMMA_16x16_N * WMMA_16x16_K;
            ...
            for (int k_w_index = 0; k_w_index < k_w; k_w_index++) {
                for (int channel_cnt = 0; channel_cnt < i_c; channel_cnt += K_step) {
                    ...
                    int4 B_row_frag[WMMA_COL_TOTAL_SZ * WMMA_16x16_N / (8 * WARP_PER_BLOCK_CHANGE)]
                                   [K_step / 128]; //[WMMA_COL_TOTAL_SZ * WMMA_16x16_N /
                                                   //WARP_PER_BLOCK_CHANGE];
...
#pragma unroll
                    for (int jj = 0;
                        ...
                        // jj need be filled
#pragma unroll
                        for (int ii = 0; ii < K_step; ii += 128) {
                            int4 *src = ((
                                int4
                                    *)(weight_tensor + matrx_B_warp_col * GEMM_K_PAD / 2 + ii + laneId_0_4 * 16 + jj * 8 * GEMM_K_PAD / 2 + (k_h_index * k_w + k_w_index) * i_c / 2 + channel_cnt / 2)); // laneId_half * 16
                            B_row_frag[jj][ii / 128] = __ldg(src);
                        }
                    }
...
#pragma unroll
                    for (int jj = 0; jj < WARP_ROW_WMMA_SZ_CHANGE; jj++) {
                        ...
                        int4 *src_0 = ((
                            int4
                                *)(input_tensor + image_offset_0 / 2 + k_h_index * (i_w * i_c / 2) + k_w_index * i_c / 2 + channel_cnt / 2 + laneId_0_4 * 16)); // laneId_half * 16
                        int4 *src_1 = ((
                            int4
                                *)(input_tensor + image_offset_1 / 2 + k_h_index * (i_w * i_c / 2) + k_w_index * i_c / 2 + channel_cnt / 2 + laneId_0_4 * 16));
                            ...
                            for (int col = 0; col < WARP_COL_WMMA_SZ_CHANGE; col++) {
                                ...
                                int8_t *b_frag0_ptr = (int8_t *)&shmem_ptr
                                     ...
                                     * MATRIX_B_SHMEM_STRIDE]; // [(warpId * WARP_COL_WMMA_SZ_CHANGE
                                                               // + col) * 64 + k +
                                                               // Matrix_B_shmem_offset];
                                int8_t *b_frag1_ptr = b_frag0_ptr + 8 * 64;
...
void lb_turing_indirect_conv_wmma_int4_ldg16_k128_matrixB_singlebuf_relu(
    ...
    workspace_t *ws)
{
    ...
    size_t sharedMM_AB_sz = N_TILED * K_TILED / 2; //(M_TILED + N_TILED) * K_TILED;
    ...
    cudaFuncSetAttribute(
        lb_turing_indirect_conv_wmma_int4_ldg16_k128_matrixB_relu_ker<
...
__global__ void lb_turing_indirect_conv_wmma_int4_ldg16_k128_relu(
    char *input_tensor, char *weight_tensor, char *output_tensor, int GEMM_M, int GEMM_N,
    int GEMM_K, int GEMM_M_PAD, int GEMM_N_PAD, int GEMM_K_PAD, float *bias, bool bias_flag,
    bool relu_flag, int i_b, int i_c, int i_h, int i_w, int k_n, int k_c, int k_h, int k_w,
    int pad_h, int pad_w, int str_h, int str_w, int out_batch, int out_channel, int out_height,
...

{
    ...
    // matrix A: [WMMA_ROW_TOTAL_SZ][WMMA_K_STEP_CHANGE][WMMA_16x16_M * WMMA_16x16_K]
    // matrix B: [WMMA_COL_TOTAL_SZ][WMMA_K_STEP_CHANGE][WMMA_16x16_N * WMMA_16x16_K]
    const int MATRIX_A_SHMEM_STRIDE = 128 / 2; // WMMA_16x16_M * WMMA_16x16_K;//WMMA_K_STEP_CHANGE *
                                               // WMMA_16x16_K + SHMEM_CONFILCT_AVOID;
    const int MATRIX_B_SHMEM_STRIDE = 128 / 2; // WMMA_16x16_N * WMMA_16x16_K;
...
#pragma unroll
                    for (int jj = 0;
                        ...
                        int4 *src = ((
                            int4
                                *)(weight_tensor + matrx_B_warp_col * GEMM_K_PAD / 2 + laneId_0_4 * 16 + jj * 8 * GEMM_K_PAD / 2 + (k_h_index * k_w + k_w_index) * i_c / 2 + channel_cnt / 2)); // laneId_half * 16
...
#pragma unroll
                    for (int jj = 0;
                        ...
                        int4 *src = ((
                            int4
                                *)(input_tensor + image_offset / 2 + k_h_index * (i_w * i_c / 2) + k_w_index * i_c / 2 + channel_cnt / 2 + laneId_0_4 * 16)); // laneId_half * 16
...
#pragma unroll
                    for (int col = 0; col < WARP_COL_WMMA_SZ_CHANGE; col++) {
                        ...
                        my_c_frag[row][col][0]
                            = mma8832(my_c_frag[row][col][0], my_A_frag0[row].z, my_B_frag0[col].z);
                        my_c_frag[row][col][0]
                            = mma8832(my_c_frag[row][col][0], my_A_frag0[row].w, my_B_frag0[col].w);
                        ...
                        my_c_frag[row][col][1]
                            = mma8832(my_c_frag[row][col][1], my_A_frag0[row].z, my_B_frag1[col].z);
                        my_c_frag[row][col][1]
                            = mma8832(my_c_frag[row][col][1], my_A_frag0[row].w, my_B_frag1[col].w);
                        ...
                        my_c_frag[row][col][2]
                            = mma8832(my_c_frag[row][col][2], my_A_frag1[row].z, my_B_frag0[col].z);
                        my_c_frag[row][col][2]
                            = mma8832(my_c_frag[row][col][2], my_A_frag1[row].w, my_B_frag0[col].w);
                        ...
                        my_c_frag[row][col][3]
                            = mma8832(my_c_frag[row][col][3], my_A_frag1[row].z, my_B_frag1[col].z);
                        my_c_frag[row][col][3]
                            = mma8832(my_c_frag[row][col][3], my_A_frag1[row].w, my_B_frag1[col].w);
                    }
                }
            }
        }
...
void lb_turing_indirect_conv_wmma_int4_ldg16_k128_singlebuf_relu(
    char *input_tensor, char *weight_tensor, char *output_tensor, int i_b, int i_c, int i_h,
    int i_w, int k_n, int k_c, int k_h, int k_w, int pad_h, int pad_w, int str_h, int str_w,
    float *bias, bool bias_flag, bool relu_flag, uint8_t bits, float *alpha, void *preComputeA,
    workspace_t *ws)
{
    ...
    size_t sharedMM_AB_sz = (M_TILED + N_TILED) * K_TILED / 2;
    ...
    dim3 blockDim(WARP_SIZE, BLOCK_COL_WARP_NUM_CHANGE, BLOCK_ROW_WARP_NUM_CHANGE);
    cudaFuncSetAttribute(
        lb_turing_indirect_conv_wmma_int4_ldg16_k128_relu<
            WARP_ROW_WMMA_SZ_CHANGE, WARP_COL_WMMA_SZ_CHANGE, WMMA_K_STEP_CHANGE, WMMA_16x16_M,
            WMMA_16x16_N, WMMA_16x16_K, BLOCK_ROW_WARP_NUM_CHANGE, BLOCK_COL_WARP_NUM_CHANGE,
            OVERLAP>,
        cudaFuncAttributeMaxDynamicSharedMemorySize, 65536);
...
template <
    ...
    int WMMA_16x16_M, int WMMA_16x16_N, int WMMA_16x16_K, int BLOCK_ROW_WARP_NUM_CHANGE,
    int BLOCK_COL_WARP_NUM_CHANGE, bool overlap>
__global__ void lb_turing_indirect_conv_wmma_int4_ldg16_k128_spilt3x3_relu(
...
template <
    int M_TILED, int N_TILED, int K_TILED, int WMMA_16x16_M, int WMMA_16x16_N, int WMMA_16x16_K,
    int BLOCK_ROW_WARP_NUM_CHANGE, int BLOCK_COL_WARP_NUM_CHANGE, bool OVERLAP>
void lb_turing_indirect_conv_wmma_int4_ldg16_k128_split3x3_singlebuf_relu(
    ...
    void *preComputeA, workspace_t *ws)
{
    ...
    dim3 blockDim(WARP_SIZE, BLOCK_COL_WARP_NUM_CHANGE, BLOCK_ROW_WARP_NUM_CHANGE);
    cudaFuncSetAttribute(
        lb_turing_indirect_conv_wmma_int4_ldg16_k128_spilt3x3_relu<
            WARP_ROW_WMMA_SZ_CHANGE, WARP_COL_WMMA_SZ_CHANGE, WMMA_K_STEP_CHANGE, WMMA_16x16_M,
            WMMA_16x16_N, WMMA_16x16_K, BLOCK_ROW_WARP_NUM_CHANGE, BLOCK_COL_WARP_NUM_CHANGE,
            OVERLAP>,
        cudaFuncAttributeMaxDynamicSharedMemorySize, 65536);
