...
#ifndef TURING_INDIRECT_CONV_WMMA_LDG16_K256_CUH
#define TURING_INDIRECT_CONV_WMMA_LDG16_K256_CUH
...
__global__ void lb_turing_indirect_conv_wmma_int8_ldg16_k256_relu(
...

{
    ...
    const int MATRIX_A_SHMEM_STRIDE = 256; // WMMA_16x16_M * WMMA_16x16_K;//WMMA_K_STEP_CHANGE *
                                           // WMMA_16x16_K + SHMEM_CONFILCT_AVOID;
    const int MATRIX_B_SHMEM_STRIDE = 256; // WMMA_16x16_N * WMMA_16x16_K;
    ...
    uint32_t laneId_0_16 = threadIdx.x & 0xF;
    uint32_t laneId_half = threadIdx.x / 16;
...

    {
        ...
        int matrx_B_warp_col
            = matrix_B_each_warp_actual_col_base + laneId_half; // laneId_even;//+ laneId_0_15;
        int matrx_A_warp_row
            = matrix_A_each_warp_actual_row_base + laneId_half; // laneId_even;//+ laneId_0_15;

        preComputeMatixA
            MatrixAInfo[WMMA_ROW_TOTAL_SZ * WMMA_16x16_M / (WARP_PER_BLOCK_CHANGE * 2)]; // 16
#pragma unroll
        for (int jj = 0; jj < WMMA_ROW_TOTAL_SZ * WMMA_16x16_M / (2 * WARP_PER_BLOCK_CHANGE);
             jj++) { // 16
            preComputeMatixA *MatrixA_info = preComputeA + matrx_A_warp_row + jj * 2;
                ...
                for (int channel_cnt = 0; channel_cnt < i_c; channel_cnt += K_step) {
                    int4 A_row_frag
                        [WMMA_ROW_TOTAL_SZ * WMMA_16x16_M
                         / (2 * WARP_PER_BLOCK_CHANGE)]; //[WMMA_ROW_TOTAL_SZ * WMMA_16x16_M /
                                                         //WARP_PER_BLOCK_CHANGE];
                    int4 B_row_frag
                        [WMMA_COL_TOTAL_SZ * WMMA_16x16_N
                         / (2 * WARP_PER_BLOCK_CHANGE)]; //[WMMA_COL_TOTAL_SZ * WMMA_16x16_N /
                                                         //WARP_PER_BLOCK_CHANGE];

                    // drag matrix weight, 1 warp 1 line(8x16), each thread drag int
#pragma unroll
                    for (int jj = 0;
                         jj < WMMA_COL_TOTAL_SZ * WMMA_16x16_N / (2 * WARP_PER_BLOCK_CHANGE);
                         jj++) {
                        // jj need be filled
                        int4 *src = ((
                            int4
                                *)(weight_tensor + matrx_B_warp_col * GEMM_K_PAD + laneId_0_16 * 16 + jj * 2 * GEMM_K_PAD + (k_h_index * k_w + k_w_index) * i_c + channel_cnt)); // laneId_half * 16
                        B_row_frag[jj] = __ldg(src);
                    }

#pragma unroll
                    for (int jj = 0;
                         jj < WMMA_ROW_TOTAL_SZ * WMMA_16x16_M / (2 * WARP_PER_BLOCK_CHANGE);
                         jj++) {
                        // jj need be filled
                        ...
                        int4 *src = ((
                            int4
                                *)(input_tensor + image_offset + k_h_index * (i_w * i_c) + k_w_index * i_c + channel_cnt + laneId_0_16 * 16)); // laneId_half * 16
...
#pragma unroll
                        for (int jj = 0;
                            ...
                            int warp_sharedmm_offset = warpId
                                ...
                                + jj * 2 * MATRIX_B_SHMEM_STRIDE;
                            int lane_sharedmm_offset = laneId_0_16 * 16
                                + laneId_half * MATRIX_B_SHMEM_STRIDE; // laneId_even * 16 +
...
#pragma unroll
                        for (int jj = 0;
                            ...
                            int warp_sharedmm_offset = warpId
                                ...
                                + jj * 2 * MATRIX_A_SHMEM_STRIDE;
                            int lane_sharedmm_offset = laneId_0_16 * 16
                                + laneId_half * MATRIX_A_SHMEM_STRIDE; // laneId_even * 16 +
                                                                       // laneId_ldg_cacheline *
                                                                       // MATRIX_B_SHMEM_STRIDE;//
...
void lb_turing_indirect_conv_wmma_int8_ldg16_k256_singlebuf_relu(
    ...
    workspace_t *ws)
{
    ...
    cudaFuncSetAttribute(
        lb_turing_indirect_conv_wmma_int8_ldg16_k256_relu<
...
template <
    ...
    int WMMA_16x16_M, int WMMA_16x16_N, int WMMA_16x16_K, int BLOCK_ROW_WARP_NUM_CHANGE,
    int BLOCK_COL_WARP_NUM_CHANGE, bool overlap = true>
__global__ void lb_turing_indirect_conv_wmma_int8_ldg16_k256_split3x3_relu(
...
void lb_turing_indirect_conv_wmma_int8_ldg16_k256_split3x3_singlebuf_relu(
    ...
    void *preComputeA, workspace_t *ws)
{
    ...
    dim3 blockDim(WARP_SIZE, BLOCK_COL_WARP_NUM_CHANGE, BLOCK_ROW_WARP_NUM_CHANGE);
    cudaFuncSetAttribute(
        lb_turing_indirect_conv_wmma_int8_ldg16_k256_split3x3_relu<
            WARP_ROW_WMMA_SZ_CHANGE, WARP_COL_WMMA_SZ_CHANGE, WMMA_K_STEP_CHANGE, WMMA_16x16_M,
            WMMA_16x16_N, WMMA_16x16_K, BLOCK_ROW_WARP_NUM_CHANGE, BLOCK_COL_WARP_NUM_CHANGE,
            OVERLAP>,
        cudaFuncAttributeMaxDynamicSharedMemorySize, 65536);
