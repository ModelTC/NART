...
#ifndef TURING_INDIRECT_CONV_WMMA_LDG16_K32_CUH
#define TURING_INDIRECT_CONV_WMMA_LDG16_K32_CUH
...
__global__ void lb_turing_indirect_conv_wmma_int8_ldg16_k32_relu(
...

{
    ...
    const int MATRIX_A_SHMEM_STRIDE = 32; // WMMA_16x16_M * WMMA_16x16_K;//WMMA_K_STEP_CHANGE *
                                          // WMMA_16x16_K + SHMEM_CONFILCT_AVOID;
    const int MATRIX_B_SHMEM_STRIDE = 32; // WMMA_16x16_N * WMMA_16x16_K;
...
#pragma unroll
                    for (int jj = 0;
                        ...
                        int4 *src = ((
                            int4
                                *)(weight_tensor + matrx_B_warp_col * GEMM_K_PAD + laneId_0_1 * 16 + jj * 16 * GEMM_K_PAD + (k_h_index * k_w + k_w_index) * i_c + channel_cnt)); // laneId_half * 16
...
#pragma unroll
                    for (int jj = 0;
                        ...
                        int4 *src = ((
                            int4
                                *)(input_tensor + image_offset + k_h_index * (i_w * i_c) + k_w_index * i_c + channel_cnt + laneId_0_1 * 16)); // laneId_half * 16
...
void lb_turing_indirect_conv_wmma_int8_ldg16_k32_singlebuf_relu(
    ...
    workspace_t *ws)
{
    ...
    cudaFuncSetAttribute(
        lb_turing_indirect_conv_wmma_int8_ldg16_k32_relu<
...
template <
    ...
    int WMMA_16x16_M, int WMMA_16x16_N, int WMMA_16x16_K, int BLOCK_ROW_WARP_NUM_CHANGE,
    int BLOCK_COL_WARP_NUM_CHANGE, bool overlap>
__global__ void lb_turing_indirect_conv_wmma_int8_ldg16_k32_split3x3_relu(
...
{
    uint32_t split_k_h_index = blockIdx.z;
    // one
    const int WMMA_ROW_TOTAL_SZ = WARP_ROW_WMMA_SZ_CHANGE * BLOCK_ROW_WARP_NUM_CHANGE;
    const int WMMA_COL_TOTAL_SZ = WARP_COL_WMMA_SZ_CHANGE * BLOCK_COL_WARP_NUM_CHANGE;
...
template <
    int M_TILED, int N_TILED, int K_TILED, int WMMA_16x16_M, int WMMA_16x16_N, int WMMA_16x16_K,
    int BLOCK_ROW_WARP_NUM_CHANGE, int BLOCK_COL_WARP_NUM_CHANGE, bool OVERLAP>
void lb_turing_indirect_conv_wmma_int8_ldg16_k32_split3x3_singlebuf_relu(
    ...
    void *preComputeA, workspace_t *ws)
{
    ...
    dim3 gridDim(matrix_split_N, matrix_split_M, k_h);

    dim3 blockDim(WARP_SIZE, BLOCK_COL_WARP_NUM_CHANGE, BLOCK_ROW_WARP_NUM_CHANGE);
    cudaFuncSetAttribute(
        lb_turing_indirect_conv_wmma_int8_ldg16_k32_split3x3_relu<
            WARP_ROW_WMMA_SZ_CHANGE, WARP_COL_WMMA_SZ_CHANGE, WMMA_K_STEP_CHANGE, WMMA_16x16_M,
            WMMA_16x16_N, WMMA_16x16_K, BLOCK_ROW_WARP_NUM_CHANGE, BLOCK_COL_WARP_NUM_CHANGE,
            OVERLAP>,
        cudaFuncAttributeMaxDynamicSharedMemorySize, 65536);
