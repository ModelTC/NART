...
op_roialignpooling_t *op_default_roialignpooling_tp_alloc(workspace_t *ws)
{
    (void)ws;
...
void op_default_roialignpooling_tp_config(op_t *op)
{
...
void op_default_roialignpooling_tp_destroy(op_t *op) { (void)op; }

void op_default_roialignpooling_tp_dealloc(op_t *op)
{
    if (NULL != op)
        free(op);
}

static void op_default_roialignpooling_run(op_t *op)
{
    ...
    for (n = 0; n < num_rois; ++n) {
        ...
        float roi_end_w = (input_1[n * roi_shape + 3] + 1.0) * spatial_scale;
        float roi_end_h = (input_1[n * roi_shape + 4] + 1.0) * spatial_scale;
...
void op_default_roialignpooling_tp_prepare(op_t *op)
{
    int i;
    ...
    case dtFLOAT32:
        op->run_func = op_default_roialignpooling_run;
        break;
    default:
        CHECK(false);
        break;
