...
op_conv_2d_wino_t *op_default_conv_2d_wino_tp_alloc(workspace_t *ws);
void op_default_conv_2d_wino_tp_config(op_t *op);
void op_default_conv_2d_wino_tp_destroy(op_t *op);
void op_default_conv_2d_wino_tp_dealloc(op_t *op);
void op_default_conv_2d_wino_tp_prepare(op_t *op);
#ifdef __cplusplus
}
#endif
op_conv_2d_wino_t *op_default_conv_2d_wino_tp_alloc(workspace_t *ws)
...
void op_default_conv_2d_wino_tp_destroy(op_t *op)
{
    if (((op_conv_2d_wino_t *)op)->aux_mem_1)
        mem_delete(((op_conv_2d_wino_t *)op)->aux_mem_1);
    if (((op_conv_2d_wino_t *)op)->aux_mem_2)
        mem_delete(((op_conv_2d_wino_t *)op)->aux_mem_2);
    if (((op_conv_2d_wino_t *)op)->temp_data)
        mem_delete(((op_conv_2d_wino_t *)op)->temp_data);
}
...
static void op_default_conv_2d_wino_blas_run(op_t *op)
{
    size_t i;
    const op_conv_2d_wino_t *conv_op = (op_conv_2d_wino_t *)op;
...
static void op_default_conv_2d_wino_bias_blas_run(op_t *op)
{
    size_t i, j, k;
    const op_conv_2d_wino_t *conv_op = (op_conv_2d_wino_t *)op;
    size_t kernel_size = conv_op->kernel_h * conv_op->kernel_w * op->input_tensors[0]->shape.dim[1];
...
static void op_default_conv_2d_wino_group_blas_run(op_t *op)
{
    size_t i, j;
    const op_conv_2d_wino_t *conv_op = (op_conv_2d_wino_t *)op;
    size_t batch_size = op->input_tensors[0]->shape.dim[0];
    ...
    for (i = 0; i < batch_size; ++i) {
        ...
        for (j = 0; j < group; ++j) {
            im2col(
                ...
                conv_op->stride_h, conv_op->stride_w, 1, 1);
...
static void op_default_conv_2d_wino_group_bias_blas_run(op_t *op)
{
    size_t i, j, k;
    const op_conv_2d_wino_t *conv_op = (op_conv_2d_wino_t *)op;
    // size_t batch_size = op->input_tensors[0]->shape.dim[0];
...
void op_default_conv_2d_wino_tp_prepare(op_t *op)
{
    printf("prepare default conv2d wino\n");
    int i;
    ...
    case dtFLOAT32:
        ...
        if (1 == group) {
            if (2 == op->input_size) {
                op->run_func = op_default_conv_2d_wino_blas_run;
            } else {
                op->run_func = op_default_conv_2d_wino_bias_blas_run;
            }
        } else {
            if (2 == op->input_size) {
                op->run_func = op_default_conv_2d_wino_group_blas_run;
            } else {
                op->run_func = op_default_conv_2d_wino_group_bias_blas_run;
            }
        }
        break;
    default:
