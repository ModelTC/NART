...
op_pow_t *op_default_pow_tp_alloc(workspace_t *ws)
{
    (void)ws;
...
void op_default_pow_tp_config(op_t *op) { (void)op; }

void op_default_pow_tp_destroy(op_t *op) { (void)op; }

void op_default_pow_tp_dealloc(op_t *op)
{
    if (NULL != op)
        free(op);
}

static void op_default_pow_run(op_t *op)
{
    ...
    if (tensor_shape_eq(&op->input_tensors[0]->shape, &op->input_tensors[1]->shape)) {
        for (i = 0; i < (int)count; ++i) {
            output_0[i] = powf(input_0[i], input_1[i]);
        }
    } else {
        ...
        if (shape_part_count(&op->input_tensors[1]->shape, 0) == 1) {
            ...
            for (i = 0; i < (int)count - 4; i = i + 4) {
                output_0[0] = powf(input_0[0], t);
                output_0[0 + 1] = powf(input_0[0 + 1], t);
                output_0[0 + 2] = powf(input_0[0 + 2], t);
                output_0[0 + 3] = powf(input_0[0 + 3], t);
                output_0 += 4;
                input_0 += 4;
            }
            for (; i < (int)count; ++i) {
                *output_0++ = powf(*input_0++, t);
            }
        } else if (
            ...
            for (uint32_t dim0 = 0; dim0 < shape.dim[0]; ++dim0) {
                const float *input_1_dim0 = input_1 + (dim0 % shape2.dim[0]) * count_except_axis0_2;
                for (uint32_t dim_middle = 0; dim_middle < count_middle0;
                     dim_middle += last_count) {
                    float *output_0_ptr = output_0 + dim_middle;
                    const float *input_0_ptr = input_0 + dim_middle;
                    const float *input_1_ptr = input_1_dim0;
                    const float *input_1_ptr_end = input_1_ptr + last_count;
                    while (input_1_ptr < input_1_ptr_end) {
                        *output_0_ptr++ = powf(*input_0_ptr++, *input_1_ptr++);
                    }
                }
...
void op_default_pow_tp_prepare(op_t *op)
{
    int i;
    ...
    case dtFLOAT32:
        op->run_func = op_default_pow_run;
        break;
    default:
        CHECK(false);
        break;
