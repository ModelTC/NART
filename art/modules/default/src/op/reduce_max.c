...
op_reducemax_t *op_default_reducemax_tp_alloc(workspace_t *ws)
{
    (void)ws;
...
void op_default_reducemax_tp_config(op_t *op)
{
    size_t len;
...
void op_default_reducemax_tp_destroy(op_t *op)
{
...
void op_default_reducemax_tp_dealloc(op_t *op)
{
    if (NULL != op) {
        free(op);
    }
...
static void reduce_max(const float *input_0, float *output_0, int cnt_i)
{
    int i;
    for (i = 0; i < cnt_i; ++i) {
...
static void op_default_reducemax_run(op_t *op)
{
    ...
    if (len == 0) {
        ...
        for (i = 0; i < (int)count; ++i) {
            output_0[0] = fmax(output_0[0], input_0[i]);
        }
        return;
    }
...
void op_default_reducemax_tp_prepare(op_t *op)
{
    int i;
    ...
    case dtFLOAT32:
        if (((op_reducemax_t *)op)->reduce == NULL) {
            ((op_reducemax_t *)op)->reduce = mem_new(cpu_mem_tp);
        }
        mem_alloc(((op_reducemax_t *)op)->reduce, count * sizeof(float));
        op->run_func = op_default_reducemax_run;
        break;
    default:
        CHECK(false);
        break;
