...
op_clip_cast_t *op_default_clip_cast_tp_alloc(workspace_t *ws);
void op_default_clip_cast_tp_config(op_t *op);
void op_default_clip_cast_tp_prepare(op_t *op);
void op_default_clip_cast_tp_destroy(op_t *op);
void op_default_clip_cast_tp_dealloc(op_t *op);
...
void op_default_clip_cast_tp_destroy(op_t *op) { (void)op; }
...
void op_default_clip_cast(D *c, T *a, const T *min, const T *max, size_t size)
{
    for (int i = 0; i < size; i++) {
        a[i] = a[i] < (*min) ? (*min) : (*max) < a[i] ? (*max) : a[i];
        c[i] = (D)a[i];
    }
}

template <typename T, typename D> static void op_default_clip_cast_run(op_t *op)
{
    ...
    if (op->input_size == 3) {
        ...
        op_default_clip_cast(output_0, input_0, min, max, count);
    } else {
        CHECK(false);
    }
}
...
void op_default_clip_cast_tp_prepare(op_t *op)
{
    ...
    if (i_dtype == dtFLOAT32 && o_dtype == dtUINT8) {
        op->run_func = op_default_clip_cast_run<float, uint8_t>;
    } else if (i_dtype == dtINT32 && o_dtype == dtUINT8) {
        op->run_func = op_default_clip_cast_run<int32_t, uint8_t>;
    } else if (i_dtype == dtINT32 && o_dtype == dtINT8) {
        op->run_func = op_default_clip_cast_run<int32_t, int8_t>;
    } else {
        CHECK(false);
    }
}
