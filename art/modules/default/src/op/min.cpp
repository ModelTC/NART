...
op_min_t *op_default_min_tp_alloc(workspace_t *ws);
void op_default_min_tp_config(op_t *op);
void op_default_min_tp_destroy(op_t *op);
void op_default_min_tp_dealloc(op_t *op);
void op_default_min_tp_prepare(op_t *op);
...
{
    (void)ws;
    op_min_t *res = (op_min_t *)malloc(sizeof(op_min_t));
    memset(res, 0, sizeof(op_min_t));
    return res;
}

void op_default_min_tp_config(op_t *op) { (void)op; }

void op_default_min_tp_destroy(op_t *op) { (void)op; }
...
template <typename T> static void op_default_min_run(op_t *op)
{
    ...
    if (tensor_shape_eq(&op->input_tensors[0]->shape, &op->input_tensors[1]->shape)) {
        for (i = 0; i < (int)count; ++i) {
            output_0[i] = std::min(input_0[i], input_1[i]);
        }
    } else {
        ...
        if (shape_part_count(&op->input_tensors[1]->shape, 0) == 1) {
            ...
            for (i = 0; i < (int)count - 4; i = i + 4) {
                output_0[0] = std::min(input_0[0], t);
                output_0[0 + 1] = std::min(input_0[0 + 1], t);
                output_0[0 + 2] = std::min(input_0[0 + 2], t);
                output_0[0 + 3] = std::min(input_0[0 + 3], t);
                output_0 += 4;
                input_0 += 4;
            }
            for (; i < (int)count; ++i) {
                *output_0++ = std::min(*input_0++, t);
            }
        } else if (
            ...
            for (uint32_t dim0 = 0; dim0 < shape.dim[0]; ++dim0) {
                ...
                for (uint32_t dim_middle = 0; dim_middle < count_middle0;
                    ...
                    while (input_1_ptr < input_1_ptr_end) {
                        *output_0_ptr++ = std::min(*input_0_ptr++, *input_1_ptr++);
                    }
                }
            ...
            for (i = 0; i < (int)count; ++i) {
                ...
                output_0[i] = std::min(input_0[id1], input_1[id2]);
            }
        }
    }
}
...
void op_default_min_tp_prepare(op_t *op)
{
    ...
    case dtFLOAT32:
        op->run_func = op_default_min_run<float>;
        break;
    case dtINT64:
        op->run_func = op_default_min_run<int64_t>;
        break;
    case dtINT32:
        op->run_func = op_default_min_run<int32_t>;
        break;
    default:
        CHECK(false);
        break;
