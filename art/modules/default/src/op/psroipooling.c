...
op_psroipooling_t *op_default_psroipooling_tp_alloc(workspace_t *ws)
{
    (void)ws;
...
void op_default_psroipooling_tp_config(op_t *op)
{
...
void op_default_psroipooling_tp_destroy(op_t *op) { (void)op; }

void op_default_psroipooling_tp_dealloc(op_t *op)
{
    if (NULL != op)
        free(op);
}

static void op_default_psroipooling_run(op_t *op)
{
...
void op_default_psroipooling_tp_prepare(op_t *op)
{
    int i;
    ...
    case dtFLOAT32:
        op->run_func = op_default_psroipooling_run;
        break;
    default:
        CHECK(false);
        break;
