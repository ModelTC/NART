...
op_sub_t *op_default_sub_tp_alloc(workspace_t *ws);
void op_default_sub_tp_config(op_t *op);
void op_default_sub_tp_destroy(op_t *op);
void op_default_sub_tp_dealloc(op_t *op);
void op_default_sub_tp_prepare(op_t *op);
#ifdef __cplusplus
} // extern "C"
#endif
...
void op_default_sub_tp_config(op_t *op) { (void)op; }

void op_default_sub_tp_destroy(op_t *op) { (void)op; }
...
template <typename T> static void op_default_sub_run(op_t *op)
{
...
void op_default_sub_tp_prepare(op_t *op)
{
    ...
    case dtFLOAT32:
        op->run_func = op_default_sub_run<float>;
        break;
    case dtINT64:
        op->run_func = op_default_sub_run<int64_t>;
        break;
    default:
        CHECK(false);
        break;
