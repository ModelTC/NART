...
op_matmul_t *op_cuda_matmul_tp_alloc(workspace_t *ws)
{
    (void)ws;
...
void op_cuda_matmul_tp_config(op_t *op) { (void)op; }

void op_cuda_matmul_tp_destroy(op_t *op) { (void)op; }

void op_cuda_matmul_tp_dealloc(op_t *op)
{
    if (NULL != op)
        free(op);
}
...
static void op_cuda_matmul_run(op_t *op)
{
    ...
    if (shape_i1.dim_size == shape_i2.dim_size) {
        ...
        for (i = 0; i < (int)count_o; i += 1) {
            ...
            CUBLAS_CHECK(cublasSgemm(
                CUDA_WORKSPACE_CUBLASHDL(op->workspace), CUBLAS_OP_N, CUBLAS_OP_N, n, m, k, &alpha,
                input_1 + offset_1 * k * n, n, input_0 + offset_0 * m * k, k, &beta, output_0, n));
            output_0 += m * n;
        }
    } else {
        // eg: 3D * 2D , 4D * 2D ...
        ...
        for (i = 0; i < (int)count_o; i += 1) {
            CUBLAS_CHECK(cublasSgemm(
                CUDA_WORKSPACE_CUBLASHDL(op->workspace), CUBLAS_OP_N, CUBLAS_OP_N, n, m, k, &alpha,
                input_1, n, input_0, k, &beta, output_0, n));
            output_0 += m * n;
            input_0 += m * k;
        }
    }
...
void op_cuda_matmul_tp_prepare(op_t *op)
{
    int i;
    ...
    case dtFLOAT32:
        op->run_func = op_cuda_matmul_run;
        break;
    default:
        CHECK(false);
        break;
