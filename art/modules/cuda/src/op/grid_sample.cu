...
op_gridsample_t *op_cuda_gridsample_tp_alloc(workspace_t *ws)
{
    (void)ws;
...
void op_cuda_gridsample_tp_config(op_t *op)
{
...
void op_cuda_gridsample_tp_destroy(op_t *op) { (void)op; }

void op_cuda_gridsample_tp_dealloc(op_t *op)
{
    if (NULL != op)
        free(op);
}
...
#define REGISTER_GRIDSAMPLE_KERNEL_IMPL(func, SCALE_FACTOR_IMPL, CLIP_IMPL, GET_VALUE_IMPL)    \
    __global__ void func(                                                                      \
        float *output, float *input, float *grid, int i_h, int i_w, int o_n, int o_c, int o_h, \
        int o_w, int count)                                                                    \
    {                                                                                          \
        CUDA_KERNEL_LOOP(index, count)                                                         \
        {                                                                                      \
            float *input_offset, *grid_offset, *output_offset;                                 \
            int n = index / (o_c * o_h * o_w);                                                 \
            int c = (index / (o_h * o_w)) % o_c;                                               \
            int h = (index / o_w) % o_h;                                                       \
            int w = index % o_w;                                                               \
                                                                                               \
            grid_offset = grid + n * o_h * o_w * 2 + 2 * (w + h * o_w);                        \
            input_offset = input + n * o_c * i_h * i_w + c * i_h * i_w;                        \
            output_offset = output + n * o_c * o_h * o_w + c * o_h * o_w + (w + h * o_w);      \
                                                                                               \
...
REGISTER_GRIDSAMPLE_KERNEL_IMPL(
    op_gridsample_4d_bilinear_pad_zero_kernel, SCALE_FACTOR, CLIP_NONE,
    TENSOR_GET_VALUE3D_PAD_ZEROS);
REGISTER_GRIDSAMPLE_KERNEL_IMPL(
    op_gridsample_4d_bilinear_pad_border_kernel, SCALE_FACTOR, CLIP, TENSOR_GET_VALUE3D);
REGISTER_GRIDSAMPLE_KERNEL_IMPL(
    op_gridsample_4d_bilinear_pad_zero_align_corner_kernel, SCALE_FACTOR_ALIGN_CORNER, CLIP_NONE,
    TENSOR_GET_VALUE3D_PAD_ZEROS);
REGISTER_GRIDSAMPLE_KERNEL_IMPL(
    op_gridsample_4d_bilinear_pad_border_align_corner_kernel, SCALE_FACTOR_ALIGN_CORNER, CLIP,
    TENSOR_GET_VALUE3D);

#define REGISTER_GRIDSAMPLE_IMPL(func)                                                 \
    static void func(op_t *op)                                                         \
    {                                                                                  \
        float *input = (float *)mem_data(op->input_tensors[0]->mem);                   \
        float *grid = (float *)mem_data(op->input_tensors[1]->mem);                    \
        ...
        int o_w = op->output_tensors[0].shape.dim[3];                                  \
                                                                                       \
        func##_kernel<<<                                                               \
            (o_n * o_c * o_h * o_w + 1024 - 1) / 1024, 1024, 0,                        \
            CUDA_WORKSPACE_STREAM(op->workspace)>>>(                                   \
            output, input, grid, i_h, i_w, o_n, o_c, o_h, o_w, o_n * o_c * o_h * o_w); \
    }

REGISTER_GRIDSAMPLE_IMPL(op_gridsample_4d_bilinear_pad_zero);
REGISTER_GRIDSAMPLE_IMPL(op_gridsample_4d_bilinear_pad_border);
REGISTER_GRIDSAMPLE_IMPL(op_gridsample_4d_bilinear_pad_zero_align_corner);
REGISTER_GRIDSAMPLE_IMPL(op_gridsample_4d_bilinear_pad_border_align_corner);

void op_cuda_gridsample_tp_prepare(op_t *op)
{
    op_gridsample_t *gridsample_op = (op_gridsample_t *)op;
    int i;
