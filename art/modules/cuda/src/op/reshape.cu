...
op_reshape_t *op_cuda_reshape_tp_alloc(workspace_t *ws)
{
    (void)ws;
...
void op_cuda_reshape_tp_config(op_t *op)
{
...
void op_cuda_reshape_tp_destroy(op_t *op) { (void)op; }

void op_cuda_reshape_tp_dealloc(op_t *op)
{
    if (NULL != op)
        free(op);
}

static void op_cuda_reshape_run(op_t *op) { (void)op; }

void op_cuda_reshape_tp_prepare(op_t *op)
{
    int i;
    ...
    case dtUNKNOWN:
        ...
        break;
    default:
        op->run_func = op_cuda_reshape_run;
        break;
    }
}
