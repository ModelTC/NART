...
op_reducemean_t *op_cuda_reducemean_tp_alloc(workspace_t *ws)
{
    (void)ws;
    op_reducemean_t *res = (op_reducemean_t *)malloc(sizeof(op_reducemean_t));
    memset(res, 0, sizeof(op_reducemean_t));
    return res;
}

void op_cuda_reducemean_tp_config(op_t *op)
{
    size_t len;
    CHECK(op_setting_array_get(
        op, SETTING_REDUCE_AXES, dtINT32, &len, &((op_reducemean_t *)op)->axes));
    (void)op;
}

void op_cuda_reducemean_tp_destroy(op_t *op)
{
    if (((op_reducemean_t *)op)->reduce != NULL) {
        mem_delete(((op_reducemean_t *)op)->reduce);
        ((op_reducemean_t *)op)->reduce = NULL;
    }
    (void)op;
}

void op_cuda_reducemean_tp_dealloc(op_t *op)
{
    if (NULL != op) {
        free(op);
    }
...
__global__ void
op_cuda_reducemean_kernel(float *output_0, float *input_0, size_t shape, size_t lowDim)
{
    ...
    if (tid == 0) {
        output[0] = (cache[0] + cache[1]) / shape;
    }
}

static void op_cuda_reducemean_run(op_t *op)
{
    ...
    float *reduce = (float *)mem_data(((op_reducemean_t *)op)->reduce);
...
void op_cuda_reducemean_tp_prepare(op_t *op)
{
    int i;
    ...
    case dtFLOAT32:
        if (((op_reducemean_t *)op)->reduce == NULL) {
            ((op_reducemean_t *)op)->reduce = mem_new(cuda_mem_tp);
        }
        mem_alloc(((op_reducemean_t *)op)->reduce, count * 2 * sizeof(float));
        op->run_func = op_cuda_reducemean_run;
        break;
    default:
        CHECK(false);
        break;
