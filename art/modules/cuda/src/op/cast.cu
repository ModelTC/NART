...
op_cast_t *op_cuda_cast_tp_alloc(workspace_t *ws);
void op_cuda_cast_tp_config(op_t *op);
void op_cuda_cast_tp_prepare(op_t *op);
void op_cuda_cast_tp_destroy(op_t *op);
void op_cuda_cast_tp_dealloc(op_t *op);
...
void op_cuda_cast_tp_destroy(op_t *op) { (void)op; }
...
template <typename T, typename D> __global__ void op_cuda_cast_kernel(D *c, const T *a, size_t size)
{
    CUDA_KERNEL_LOOP(i, size) { c[i] = (D)a[i]; }
}
template <typename T, typename D> static void op_cuda_cast_run(op_t *op)
{
...
void op_cuda_cast_tp_prepare(op_t *op)
{
    ...
    if (i_dtype == dtFLOAT32 && o_dtype == dtUINT8) {
        op->run_func = op_cuda_cast_run<float, uint8_t>;
    } else if (i_dtype == dtINT32 && o_dtype == dtUINT8) {
        op->run_func = op_cuda_cast_run<int32_t, uint8_t>;
    } else if (i_dtype == dtINT32 && o_dtype == dtINT8) {
        op->run_func = op_cuda_cast_run<int32_t, int8_t>;
    } else {
        CHECK(false);
    }
}
