...
op_reducel2_t *op_cuda_reducel2_tp_alloc(workspace_t *ws)
{
    (void)ws;
    op_reducel2_t *res = (op_reducel2_t *)malloc(sizeof(op_reducel2_t));
    memset(res, 0, sizeof(op_reducel2_t));
    return res;
}

void op_cuda_reducel2_tp_config(op_t *op)
{
    size_t len;
    CHECK(
        op_setting_array_get(op, SETTING_REDUCE_AXES, dtINT32, &len, &((op_reducel2_t *)op)->axes));
    (void)op;
}

void op_cuda_reducel2_tp_destroy(op_t *op)
{
    if (((op_reducel2_t *)op)->reduce != NULL) {
        mem_delete(((op_reducel2_t *)op)->reduce);
        ((op_reducel2_t *)op)->reduce = NULL;
    }
    (void)op;
}

void op_cuda_reducel2_tp_dealloc(op_t *op)
{
    if (NULL != op) {
        free(op);
    }
...
__global__ void
op_cuda_reducel2_kernel(float *output_0, float *input_0, size_t shape, size_t lowDim)
{
    ...
    cache[tid] = bInput[lowDim * tid] * bInput[lowDim * tid];
    __syncthreads();
    if (shape > 1024) {
        ...
        for (int i = 1; i < cycle; i++) {
            ...
            if (tid < calShape) {
                cache[tid] += bInput[lowDim * tid + i * 1024 * lowDim]
                    * bInput[lowDim * tid + i * 1024 * lowDim];
            }
        }
    }
    __syncthreads();
    ...
    if (tid == 0) {
        output[0] = sqrt(cache[0] + cache[1]);
    }
}

static void op_cuda_reducel2_run(op_t *op)
{
    ...
    float *reduce = (float *)mem_data(((op_reducel2_t *)op)->reduce);
...
void op_cuda_reducel2_tp_prepare(op_t *op)
{
    int i;
    ...
    case dtFLOAT32:
        if (((op_reducel2_t *)op)->reduce == NULL) {
            ((op_reducel2_t *)op)->reduce = mem_new(cuda_mem_tp);
        }
        mem_alloc(((op_reducel2_t *)op)->reduce, count * 2 * sizeof(float));
        op->run_func = op_cuda_reducel2_run;
        break;
    default:
        CHECK(false);
        break;
