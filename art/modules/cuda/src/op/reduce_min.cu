...
op_reducemin_t *op_cuda_reducemin_tp_alloc(workspace_t *ws)
{
    (void)ws;
...
void op_cuda_reducemin_tp_config(op_t *op)
{
    size_t len;
...
void op_cuda_reducemin_tp_destroy(op_t *op)
{
...
void op_cuda_reducemin_tp_dealloc(op_t *op)
{
    if (NULL != op) {
        free(op);
    }
...
__global__ void
op_cuda_reducemin_kernel(float *output_0, float *input_0, size_t shape, size_t lowDim)
{
    ...
    if (shape > 1024) {
        ...
        for (int i = 1; i < cycle; i++) {
            ...
            if (tid < calShape) {
                cache[tid] = fmin(cache[tid], bInput[lowDim * tid + i * 1024 * lowDim]);
            }
        }
    }
    __syncthreads();
    ...
    for (int stride = (blockDim.x + 1) >> 1; stride > 1; stride = (stride + 1) >> 1) {
        if ((tid + stride) < count) {
            cache[tid] = fmin(cache[tid], cache[stride + tid]);
        }
        count = stride;
        __syncthreads();
    }
    if (tid == 0) {
        output[0] = fmin(cache[0], cache[1]);
    }
}

static void op_cuda_reducemin_run(op_t *op)
{
...
void op_cuda_reducemin_tp_prepare(op_t *op)
{
    int i;
    ...
    case dtFLOAT32:
        if (((op_reducemin_t *)op)->reduce == NULL) {
            ((op_reducemin_t *)op)->reduce = mem_new(cuda_mem_tp);
        }
        mem_alloc(((op_reducemin_t *)op)->reduce, count * sizeof(float) * 2);
        op->run_func = op_cuda_reducemin_run;
        break;
    default:
        CHECK(false);
        break;
