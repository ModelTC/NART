...
// purpose : implement the psroi pooling layer in cuda (reference to caffeinfer's psroipooling.cu)
...
op_psroipooling_t *op_cuda_psroipooling_tp_alloc(workspace_t *ws)
{
    (void)ws;
    op_psroipooling_t *res = (op_psroipooling_t *)malloc(sizeof(op_psroipooling_t));
    memset(res, 0, sizeof(op_psroipooling_t));
    return res;
}

void op_cuda_psroipooling_tp_config(op_t *op)
{
    CHECK(op_setting_single_get(
        op, SETTING_PSROIPOOLING_SPATIAL_SCALE, dtFLOAT32,
        &((op_psroipooling_t *)op)->spatial_scale));
    CHECK(op_setting_single_get(
        op, SETTING_PSROIPOOLING_OUTPUT_DIM, dtUINT32, &((op_psroipooling_t *)op)->output_dim));
    CHECK(op_setting_single_get(
        op, SETTING_PSROIPOOLING_GROUP_SIZE, dtUINT32, &((op_psroipooling_t *)op)->group_size));
    CHECK(op_setting_single_get(
        op, SETTING_PSROIPOOLING_SAMPLE_NUM, dtUINT32, &((op_psroipooling_t *)op)->sample_num));
}

void op_cuda_psroipooling_tp_destroy(op_t *op) { (void)op; }

void op_cuda_psroipooling_tp_dealloc(op_t *op)
{
    if (NULL != op)
        free(op);
}

__global__ void op_cuda_psroipooling_kernel(
...
static void op_cuda_psroipooling_run(op_t *op)
{
    op_psroipooling_t *pool_op = (op_psroipooling_t *)op;
    ...
    size_t count = group_size * group_size * output_dim * num_rois;

    op_cuda_psroipooling_kernel<<<
...
void op_cuda_psroipooling_tp_prepare(op_t *op)
{
    int i;
    ...
    case dtFLOAT32:
        op->run_func = op_cuda_psroipooling_run;
        break;
    default:
        CHECK(false);
        break;
