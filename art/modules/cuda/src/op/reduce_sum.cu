...
op_reducesum_t *op_cuda_reducesum_tp_alloc(workspace_t *ws)
{
    (void)ws;
    op_reducesum_t *res = (op_reducesum_t *)malloc(sizeof(op_reducesum_t));
    memset(res, 0, sizeof(op_reducesum_t));
    return res;
}

void op_cuda_reducesum_tp_config(op_t *op)
{
    size_t len;
    CHECK(op_setting_array_get(
        op, SETTING_REDUCE_AXES, dtINT32, &len, &((op_reducesum_t *)op)->axes));
    (void)op;
}

void op_cuda_reducesum_tp_destroy(op_t *op)
{
    if (((op_reducesum_t *)op)->reduce != NULL) {
        mem_delete(((op_reducesum_t *)op)->reduce);
        ((op_reducesum_t *)op)->reduce = NULL;
    }
    (void)op;
}

void op_cuda_reducesum_tp_dealloc(op_t *op)
{
    if (NULL != op) {
        free(op);
    }
...
__global__ void
op_cuda_reducesum_kernel(float *output_0, float *input_0, size_t shape, size_t lowDim)
{
    // CUDA_KERNEL_LOOP(i, size) {
    //     output_0[i] = output_0[i] + input_0[i];
    // }
    ...
    // copy data[0:blockDim.x-1] from global memory to shared memory
    cache[tid] = bInput[lowDim * tid];
    __syncthreads();
    // reduce data's shape to 1024 when shape > 1024
    if (shape > 1024) {
        int cycle = (shape + 1023) / 1024;
        for (int i = 1; i < cycle; i++) {
            int calShape = ((i == cycle - 1) && (shape % 1024)) ? (shape % 1024) : 1024;
            if (tid < calShape) {
                cache[tid] += bInput[lowDim * tid + i * 1024 * lowDim];
            }
        }
    }
    __syncthreads();
    ...
    for (int stride = (blockDim.x + 1) >> 1; stride > 1; stride = (stride + 1) >> 1) {
        if ((tid + stride) < count) {
            cache[tid] += cache[stride + tid];
        }
        // count means number that join next reduction
        count = stride;
        __syncthreads();
    }
    if (tid == 0) {
        output[0] = cache[0] + cache[1];
    }
}

static void op_cuda_reducesum_run(op_t *op)
{
    ...
    float *reduce = (float *)mem_data(((op_reducesum_t *)op)->reduce);
    ...
    CUDA_CHECK(cudaMemcpyAsync(
        ...
        CUDA_WORKSPACE_STREAM(op->workspace)));
    // printf("%f %f",*output_0,*(output_0+1));
}
void op_cuda_reducesum_tp_prepare(op_t *op)
{
    int i;
    ...
    case dtFLOAT32:
        if (((op_reducesum_t *)op)->reduce == NULL) {
            ((op_reducesum_t *)op)->reduce = mem_new(cuda_mem_tp);
        }
        mem_alloc(((op_reducesum_t *)op)->reduce, count * 2 * sizeof(float));
        op->run_func = op_cuda_reducesum_run;
        break;
    default:
        CHECK(false);
        break;
