...
op_sub_t *op_cuda_sub_tp_alloc(workspace_t *ws)
{
    (void)ws;
    op_sub_t *res = (op_sub_t *)malloc(sizeof(op_sub_t));
    memset(res, 0, sizeof(op_sub_t));
    return res;
}

void op_cuda_sub_tp_config(op_t *op) { (void)op; }

void op_cuda_sub_tp_destroy(op_t *op)
{
    if (((op_sub_t *)op)->shape_dim != NULL) {
        mem_delete(((op_sub_t *)op)->shape_dim);
        ((op_sub_t *)op)->shape_dim = NULL;
    }
    if (((op_sub_t *)op)->shape_dim1 != NULL) {
        mem_delete(((op_sub_t *)op)->shape_dim1);
        ((op_sub_t *)op)->shape_dim1 = NULL;
    }
    if (((op_sub_t *)op)->shape_dim2 != NULL) {
        mem_delete(((op_sub_t *)op)->shape_dim2);
        ((op_sub_t *)op)->shape_dim2 = NULL;
    }
    (void)op;
}

void op_cuda_sub_tp_dealloc(op_t *op)
{
    if (NULL != op)
        free(op);
}

__global__ void op_cuda_sub_kernel(float *c, const float *a, const float *b, size_t size)
{
    CUDA_KERNEL_LOOP(i, size) { c[i] = a[i] - b[i]; }
}

__global__ void op_cuda_sub_broadcast_kernel(
...
static void op_cuda_sub_run(op_t *op)
{
...
void op_cuda_sub_tp_prepare(op_t *op)
{
    ...
    case dtFLOAT32:
        if (((op_sub_t *)op)->shape_dim == NULL) {
            ((op_sub_t *)op)->shape_dim = mem_new(cuda_mem_tp);
        }
        mem_alloc(((op_sub_t *)op)->shape_dim, count * sizeof(uint32_t));
        if (((op_sub_t *)op)->shape_dim1 == NULL) {
            ((op_sub_t *)op)->shape_dim1 = mem_new(cuda_mem_tp);
        }
        mem_alloc(((op_sub_t *)op)->shape_dim1, count * sizeof(uint32_t));
        if (((op_sub_t *)op)->shape_dim2 == NULL) {
            ((op_sub_t *)op)->shape_dim2 = mem_new(cuda_mem_tp);
        }
        mem_alloc(((op_sub_t *)op)->shape_dim2, count * sizeof(uint32_t));
        op->run_func = op_cuda_sub_run;
        break;
    default:
        CHECK(false);
        break;
