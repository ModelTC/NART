...
op_elu_t *op_cuda_elu_tp_alloc(workspace_t *ws)
{
    (void)ws;
...
void op_cuda_elu_tp_config(op_t *op)
{
    op_elu_t *elu_op = reinterpret_cast<op_elu_t *>(op);
    op_setting_single_get(op, SETTING_ELU_ALPHA, dtFLOAT32, &(elu_op->alpha));
}

void op_cuda_elu_tp_destroy(op_t *op) { (void)op; }

void op_cuda_elu_tp_dealloc(op_t *op)
{
    if (NULL != op)
        free(op);
}

__global__ void op_cuda_elu_kernel(float *b, const float *a, size_t size, const float alpha)
{
    CUDA_KERNEL_LOOP(i, size)
    {
        float x = a[i];
        b[i] = x >= 0 ? x : alpha * (expf(x) - 1.0);
    }
}

static void op_cuda_elu_run(op_t *op)
{
    op_elu_t *elu_op = reinterpret_cast<op_elu_t *>(op);
    ...
    op_cuda_elu_kernel<<<
        ...
        output, input, count, alpha);
}

void op_cuda_elu_tp_prepare(op_t *op)
{
    int i;
