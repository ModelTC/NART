...
op_pow_t *op_cuda_pow_tp_alloc(workspace_t *ws)
{
    (void)ws;
    op_pow_t *res = (op_pow_t *)malloc(sizeof(op_pow_t));
    memset(res, 0, sizeof(op_pow_t));
    return res;
}

void op_cuda_pow_tp_config(op_t *op) { (void)op; }

void op_cuda_pow_tp_destroy(op_t *op)
{
    if (((op_pow_t *)op)->shape_dim != NULL) {
        mem_delete(((op_pow_t *)op)->shape_dim);
        ((op_pow_t *)op)->shape_dim = NULL;
    }
    if (((op_pow_t *)op)->shape_dim1 != NULL) {
        mem_delete(((op_pow_t *)op)->shape_dim1);
        ((op_pow_t *)op)->shape_dim1 = NULL;
    }
    if (((op_pow_t *)op)->shape_dim2 != NULL) {
        mem_delete(((op_pow_t *)op)->shape_dim2);
        ((op_pow_t *)op)->shape_dim2 = NULL;
    }
    (void)op;
}

void op_cuda_pow_tp_dealloc(op_t *op)
{
    if (NULL != op)
        free(op);
}

__global__ void op_cuda_pow_kernel(float *c, const float *a, const float *b, size_t size)
{
    CUDA_KERNEL_LOOP(i, size) { c[i] = powf(a[i], b[i]); }
}

__global__ void op_cuda_pow_broadcast_kernel(
    ...
    uint32_t *shape_dim1, uint32_t *shape_dim2, uint32_t dim_size)
{
    CUDA_KERNEL_LOOP(i, size)
    {
        ...
        output_0[i] = powf(input_0[id1], input_1[id2]);
    }
}

static void op_cuda_pow_run(op_t *op)
{
...
void op_cuda_pow_tp_prepare(op_t *op)
{
    ...
    case dtFLOAT32:
        if (((op_pow_t *)op)->shape_dim == NULL) {
            ((op_pow_t *)op)->shape_dim = mem_new(cuda_mem_tp);
        }
        mem_alloc(((op_pow_t *)op)->shape_dim, count * sizeof(uint32_t));
        if (((op_pow_t *)op)->shape_dim1 == NULL) {
            ((op_pow_t *)op)->shape_dim1 = mem_new(cuda_mem_tp);
        }
        mem_alloc(((op_pow_t *)op)->shape_dim1, count * sizeof(uint32_t));
        if (((op_pow_t *)op)->shape_dim2 == NULL) {
            ((op_pow_t *)op)->shape_dim2 = mem_new(cuda_mem_tp);
        }
        mem_alloc(((op_pow_t *)op)->shape_dim2, count * sizeof(uint32_t));
        op->run_func = op_cuda_pow_run;
        break;
    default:
        CHECK(false);
        break;
