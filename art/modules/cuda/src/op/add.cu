...
op_add_t *op_cuda_add_tp_alloc(workspace_t *ws);
void op_cuda_add_tp_config(op_t *op);
void op_cuda_add_tp_prepare(op_t *op);
void op_cuda_add_tp_destroy(op_t *op);
void op_cuda_add_tp_dealloc(op_t *op);
...
void op_cuda_add_tp_config(op_t *op) { (void)op; }

void op_cuda_add_tp_destroy(op_t *op)
{
    if (((op_add_t *)op)->shape_dim != NULL) {
        mem_delete(((op_add_t *)op)->shape_dim);
        ((op_add_t *)op)->shape_dim = NULL;
    }
    if (((op_add_t *)op)->shape_dim1 != NULL) {
        mem_delete(((op_add_t *)op)->shape_dim1);
        ((op_add_t *)op)->shape_dim1 = NULL;
    }
    if (((op_add_t *)op)->shape_dim2 != NULL) {
        mem_delete(((op_add_t *)op)->shape_dim2);
        ((op_add_t *)op)->shape_dim2 = NULL;
    }
    (void)op;
}
...
template <typename T> __global__ void op_cuda_add_kernel(T *c, const T *a, const T *b, size_t size)
{
    CUDA_KERNEL_LOOP(i, size) { c[i] = a[i] + b[i]; }
}

template <typename T>
__global__ void op_cuda_add_broadcast_kernel(
...
template <typename T> static void op_cuda_add_run(op_t *op)
{
    ...
    if (tensor_shape_eq(&op->input_tensors[0]->shape, &op->input_tensors[1]->shape)) {
        ...
        uint32_t *shape_dim = (uint32_t *)mem_data(((op_add_t *)op)->shape_dim);
        CUDA_CHECK(cudaMemcpyAsync(
            shape_dim, pinned_shapes, sizeof(uint32_t) * shape.dim_size, cudaMemcpyHostToDevice,
            CUDA_WORKSPACE_STREAM(op->workspace)));
        uint32_t *shape_dim1 = (uint32_t *)mem_data(((op_add_t *)op)->shape_dim1);
        CUDA_CHECK(cudaMemcpyAsync(
...
void op_cuda_add_tp_prepare(op_t *op)
{
    ...
    if (((op_add_t *)op)->shape_dim == NULL) {
        ((op_add_t *)op)->shape_dim = mem_new(cuda_mem_tp);
    }
    mem_alloc(((op_add_t *)op)->shape_dim, count * sizeof(uint32_t));
    if (((op_add_t *)op)->shape_dim1 == NULL) {
        ((op_add_t *)op)->shape_dim1 = mem_new(cuda_mem_tp);
    }
    mem_alloc(((op_add_t *)op)->shape_dim1, count * sizeof(uint32_t));
    if (((op_add_t *)op)->shape_dim2 == NULL) {
        ((op_add_t *)op)->shape_dim2 = mem_new(cuda_mem_tp);
    }
    mem_alloc(((op_add_t *)op)->shape_dim2, count * sizeof(uint32_t));

    switch (op->input_tensors[0]->dtype) {
    case dtFLOAT32:
        op->run_func = op_cuda_add_run<float>;
        break;
    case dtINT32:
        op->run_func = op_cuda_add_run<int32_t>;
        break;
    default:
        CHECK(false);
        break;
