...
op_reduceprod_t *op_cuda_reduceprod_tp_alloc(workspace_t *ws)
{
    (void)ws;
...
void op_cuda_reduceprod_tp_config(op_t *op)
{
    size_t len;
...
void op_cuda_reduceprod_tp_destroy(op_t *op)
{
...
void op_cuda_reduceprod_tp_dealloc(op_t *op)
{
    if (NULL != op) {
        free(op);
    }
...
__global__ void
op_cuda_reduceprod_kernel(float *output_0, float *input_0, size_t shape, size_t lowDim)
{
...
static void op_cuda_reduceprod_run(op_t *op)
{
...
void op_cuda_reduceprod_tp_prepare(op_t *op)
{
    int i;
    ...
    case dtFLOAT32:
        if (((op_reduceprod_t *)op)->reduce == NULL) {
            ((op_reduceprod_t *)op)->reduce = mem_new(cuda_mem_tp);
        }
        mem_alloc(((op_reduceprod_t *)op)->reduce, count * 2 * sizeof(float));
        op->run_func = op_cuda_reduceprod_run;
        break;
    default:
        CHECK(false);
        break;
